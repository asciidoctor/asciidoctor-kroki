      ...
      it('should convert a PlantUML diagram and resolve include relative to base directory', async () => {
        ...
        const diagramText = fs
          ...
          .replace(
            /^!include (.*)\r?\n/m,
            `${fs.readFileSync(
              fixturePath('plantuml', 'styles', 'general.iuml'),
              'utf8',
            )}\n`,
          )
        const input = `plantuml::${file}[svg,role=sequence]`
        ...
        const diagramText = fs
          ...
          .replace(
            ...
            fs.readFileSync(
              ...
              '\n' +
              fs.readFileSync(
                fixturePath('plantuml', 'styles', 'note.iuml'),
                'utf8',
              ) +
              '\n' +
              fs.readFileSync(
                fixturePath('plantuml', 'styles', 'sequence.iuml'),
                'utf8',
              ) +
              '\n',
          )
      ...
      it('should convert a PlantUML diagram and resolve includes from configured kroki-plantuml-include-paths attribute with multiple paths', async () => {
        ...
        const diagramText = fs
          ...
          .replace(
            /^!include base.iuml\r?\n/m,
            `${fs.readFileSync(
              fixturePath('plantuml', 'include', 'base.iuml'),
              'utf8',
            )}\n`,
          )
          .replace(
        ...
        const html = await convert(input, {
          ...
          attributes: {
            'kroki-plantuml-include-paths':
              fixturePath('plantuml', 'styles') +
              ospath.delimiter +
              fixturePath('plantuml', 'include'),
            'kroki-server-url': krokiServerUrl,
          },
          base_dir: fixturePath(),
        })
...
`
        ...
        const html = fs.readFileSync(
          ospath.join(
            ...
            'kroki',
            'to_dir',
            'to-dir-option-with-imagedir-attr.html',
          ),
          'utf8',
        )
        assertContains(
...
`
      ...
      it('should convert a Vega-Lite diagram and resolve data.url relative to the diagram file', async () => {
        ...
        const values = fs.readFileSync(
          fixturePath('docs', 'diagrams', 'data', 'seattle-weather.csv'),
          'utf8',
        )
      ...
      describe('Default options', () => {
        const defaultOptionsFixtures = [
          ...
          },
          {
            format: 'svg',
            formatLocation:
              'page attribute overriding previous png page attribute',
